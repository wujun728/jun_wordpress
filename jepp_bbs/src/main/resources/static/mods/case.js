...
layui.define(['laypage', 'fly'], function(exports){
  ...
  var active = {
      ...
      layer.open({
        ...
        ,content: ['<ul class="layui-form" style="margin: 20px;">'
          ...
          ,'<li class="layui-form-item layui-form-text">'
            ...
            ,'<div class="layui-input-block layui-form-text">'
              ,'<textarea required name="description" lay-verify="required" autocomplete="off" placeholder="大致介绍你的项目，也可以阐述你在该项目中使用 layui 的感受\n10-60个字" class="layui-textarea"></textarea>'
            ,'</div>'
          ,'</li>'
          ,'<li class="layui-form-item">'
        ...
        ,success: function(layero, index){
          ...
          upload.render({
            ...
            ,done: function(res){
              if(res.code == 0){
                ...
                preview.html('<a href="'+ res.attachment_list[0].filePath +'" target="_blank" style="color: #5FB878;">封面已上传，点击可预览</a>');
              } else {
          ...
          form.render('checkbox').on('submit(pushCase)', function(data){
            ...
            fly.json('/example/add/', data.field, function(res){
              layer.close(index);
              layer.alert(res.msg, {
                icon: 1
              })
    ...
    ,praise: function(othis){
      ...
      fly.json('/example/'+li.data('id')+'/praise/', {
        id: li.data('id')
    ...
    ,showPraise: function(othis){
      ...
      fly.json('/example/'+li.data('id')+'/praiseUser/', {
        id: li.data('id')
      }, function(res){
        var html = '';
        layer.open({
          ...
          ,content: function(){
            layui.each(res.data, function(_, item){
              html += '<li><a href="/user/home/'+ item.user.id +'/" target="_blank" title="'+ item.user.realname +'"><img src="'+ item.user.portrait +'"></a></li>'
            });
