...
layui.define('fly', function(exports){
  ...
  gather.jieAdmin = {
    ...
    del: function(div){
      layer.confirm('确认删除该求解么？', function(index){
        layer.close(index);
        fly.json('/posts/' +div.data('id') +'/delete', {
          id: div.data('id')
        }, function(res){
          if(res.code === 0){
            location.href = '/posts/';
    ...
    ,set: function(div){
      var othis = $(this);
      fly.json('/posts/' +div.data('id') +'/set', {
        id: div.data('id')
    ...
    ,collect: function(div){
      ...
      fly.json('/posts/comment/collection/'+ type +'/', {
        postsId: div.data('id')
      }, function(res){
        if(type === 'add'){
  ...
  var asyncRender = function(){
    ...
    if(jieAdmin[0] && layui.cache.user.uid != ''){
      fly.json('/posts/comment/collection/find/', {
        postsId: div.data('id')
      }, function(res){
        jieAdmin.append('<span class="layui-btn layui-btn-xs jie-admin '+ (res.collection ? 'layui-btn-danger' : '') +'" type="collect" data-type="'+ (res.collection ? 'remove' : 'add') +'">'+ (res.collection ? '取消收藏' : '收藏') +'</span>');
      });
    }
  }();
  ...
  gather.jiedaActive = {
    zan: function(li){ //赞
      ...
      fly.json('/posts/comment/' +li.data('id') +'/praise', {
        ok: ok
        ,id: li.data('id')
      }, function(res){
        if(res.code === 0){
    ...
    ,accept: function(li){ //采纳
      ...
      layer.confirm('是否采纳该回答为最佳答案？', function(index){
        layer.close(index);
        fly.json('/posts/comment/' +li.data('id') +'/accept', {
          id: li.data('id')
        }, function(res){
          if(res.code === 0){
            $('.jieda-accept').remove();
    ...
    ,edit: function(li){ //编辑
      fly.json('/posts/comment/' +li.data('id') +'/get', {
        id: li.data('id')
      }, function(res){
        var data = res;
        layer.prompt({
          ...
          fly.json('/posts/comment/' +li.data('id') +'/update', {
            id: li.data('id')
            ,content: value
          }, function(res){
            layer.close(index);
    ...
    ,del: function(li){ //删除
      layer.confirm('确认删除该回答么？', function(index){
        layer.close(index);
        fly.json('/posts/comment/' +li.data('id') +'/delete', {
          id: li.data('id')
        }, function(res){
          if(res.code === 0){
            var count = dom.jiedaCount.text()|0;
